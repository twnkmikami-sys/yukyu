...
with st.sidebar:
    ...
    # アカウント関連メニュー選択
    menu_account = st.selectbox(
        "アカウント",
        account_menu_list
    )

    # ログアウト処理
    if menu_account == "ログアウト":
...
# =====================================
# パスワード変更処理：メニューから呼び出し
# =====================================
if menu_account == "パスワード変更":
    st.header("パスワード変更")
    ...
    if st.button(
        ...
        if new_pass != new_pass2:
            ...
            if len(current_user_df) == 0:
                ...
                db_pass = current_user_df.iloc[0]["password"]
                # 現在のパスワード確認
    ...
    if st.button("全社員Excel出力", key="all_individual_excel"):
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for idx, emp_row in all_employee_df.iterrows():
                ...
                ws["B9"] = "取得日数"
                row_no = 10
                for _, row in history_df_export.iterrows():
                    ws.cell(row=row_no, column=1).value = row["leave_date"]
                    ws.cell(row=row_no, column=2).value = row["leave_days"]
                    row_no += 1
                ...
                for _, row in grant_df.iterrows():
                    if row["expired_flag"] == "消滅済":
                        continue
                    expire_date = datetime.strptime(row["expire_date"], "%Y-%m-%d").date()
                    remain_days_calc = float(row["grant_days"]) - float(row["used_days"])
                    if remain_days_calc <= 0:
                        continue
                    days_left = (expire_date - today_date).days
                    if days_left <= 30:
                        ws[f"D{warning_row}"] = "期限接近"
                        ws[f"E{warning_row}"] = str(expire_date)
                        ws[f"F{warning_row}"] = remain_days_calc
                        ws[f"D{warning_row}"].fill = yellow_fill
                        ws[f"E{warning_row}"].fill = yellow_fill
                        ws[f"F{warning_row}"].fill = yellow_fill
                        warning_row += 1
                ...
                # Excelをメモリ上に保存してZIPに追加
                individual_buffer = io.BytesIO()
                wb.save(individual_buffer)
                individual_buffer.seek(0)
                file_name = f"{emp_name}_有給台帳.xlsx"
                zipf.writestr(file_name, individual_buffer.read())
                # ========== 【追加】ここまでExcel作成処理 ==========

        # 処理完了後、バッファを保存
        zip_buffer.seek(0)
        st.session_state["all_zip_data"] = zip_buffer
        st.session_state["all_zip_filename"] = "全社員_有給台帳一括.zip"
        st.session_state["preview_all"] = pd.DataFrame(preview_summary)
        st.success("✅ 全社員分のExcel作成完了　→　下で内容を確認し、ZIPダウンロードできます")

    # --- プレビュー表示エリア ---
    if st.session_state["preview_all"] is not None:
        st.subheader("👀 出力内容プレビュー（一覧）")
        st.dataframe(
            st.session_state["preview_all"],
            hide_index=True,
            use_container_width=True
        )

        # 要注意者のみフィルタ表示
        warning_all = st.session_state["preview_all"][
            st.session_state["preview_all"]["状態"] == "要注意"
        ]
        if len(warning_all) > 0:
            st.warning(f"⚠️ 出勤率80%未達の要注意社員：{len(warning_all)}名")
            st.dataframe(warning_all, hide_index=True)

    # --- ダウンロードボタン ---
    if st.session_state["all_zip_data"] is not None:
        st.download_button(
            label="📥 全社員一括ZIPダウンロード",
            data=st.session_state["all_zip_data"],
            file_name=st.session_state["all_zip_filename"],
